...
- 備考:
  ...
  リクエストの `email` は、`POST /v1/auth/send-verification-code`（`type=register`）が発码した
  宛先アドレスと一致しなければならない（大文字小文字は区別せず、前後空白は無視）。
  `email` が未指定または不一致の場合は `400 VERIFICATION_CODE_ERROR`
  （メッセージ `验证码与邮箱不匹配，请重新获取`）を返し、**コードと误输入回数カウンタを消費しない**
  ため、有効期限までは正しいアドレスで再試行できる。
...
- 備考:
  ...
  6 桁の認証コードはメールで配送され、有効期限は 5 分。件名にコードは含まれない。
  `email` は `type=register` では必須（その宛先へ送信）。`type=login` では無視され、
  アカウントに紐づくメールアドレスへ送信される。
  発码した宛先メールは 5 分間記録され、`POST /v1/auth/register` 側で同一アドレスを要求する
  ために使われる（同エンドポイント参照）。
  レート制限：同一 IP から 60 秒あたり 5 回を超えると `429`（エンドポイント単位のスロットル）、
  同一宛先の 60 秒クールダウン中も `429`。
  再送すると、その電話番号・type の误输入回数カウンタはリセットされる。
  エラー：宛先メールを解決できない場合（register で `email` なし、または login で
  メール未登録のアカウント）は `400 RECIPIENT_EMAIL_MISSING`。`type=register` で
  そのアドレスが既存アカウントに紐づいている場合は `409 EMAIL_EXISTS`
  （`邮箱 <email> 已存在`）。発码前の重複チェックは大文字小文字を区別せずに照合し
  （前後空白は無視）、DB の `email` unique 制約自体は大文字小文字を区別する。
  したがって大文字小文字だけが異なるアドレスは、送信・保存の前にこのチェックで拒否される。
  この重複チェックはコードの生成・送信・保存より前に実行されるため、拒否時はメール送信も
  Redis 書込（コード / クールダウン / 発码宛先バインディング）も一切行われない。
  60 秒クールダウン中は `429` がこの `409` より優先される（クールダウンが先に判定される）。
  メール送信に失敗した場合は `502 EXTERNAL_SERVICE_ERROR`（この場合コードは保存されない）。
